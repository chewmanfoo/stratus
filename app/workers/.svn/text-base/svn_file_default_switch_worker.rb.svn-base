class SvnFileDefaultSwitchWorker < GenericWorker
  require 'tmpdir'
  ...
  def switch_column(file)
    ...
    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> find switch_column -> found switch_column is '#{@num}'")
...
# imperative: there is only one line in servers.csv that matches server
    ...
    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> DEBUG: find line_containing_server -> found line is '#{@server_line}'")
# fix the line so that it only contains one line - this is a hack!
    @server_line = @server_line.match(/(?:[^|]*\|){#{@num_col - 1}}[^|]*/).to_s

    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> find line_containing_server -> found line is '#{@server_line}'")
...
#go ahead and remove old versions of switches
      ...
      # cleanup - remove leading, trailing commas
      @switches.sub!(/^[,]?/, "").sub!(/[,]?$/, "")
    end
...
# use hash with name = switch name and value = version.tar 
    ...
    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> process_file -> executing: replacing '#{@oldline}' with '#{@newline}' in file '#{file.to_s}'")
  end
  ...
  def fix_double_commas(file)
    @result = `sed -i 's/,,/,/g' #{file}`
    @result2 = `sed -i 's/,|/|/g' #{file}`
    @result3 = `sed -i 's/, |/ |/g' #{file}`
  end
  ...
  def do_perform(event_id)
    ...
    raise UserTerminatedException unless check_terminated("SvnFileDefaultSwitchWorker", @event)

    raise OutOfOrderException unless check_order("SvnFileDefaultSwitchWorker", @event, "to_default_after")

    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> start")

    if @config.debug_mode?
      @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> debug mode")
    else
      ...
      Dir.mktmpdir do |dir|
        ...
        raise UnixFailedExitException unless perform_unix("SvnFileDefaultSwitchWorker", @event, @task)
    ...
    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> end")
    @event.class.log_result(@event, "SvnFileDefaultSwitchWorker::perform -> SUCCESS: [#{@result}]")
    @event.syncmanifest_again!
  end
