class ApplicationReportWorker < GenericWorker
  require 'tmpdir'
  ...
  def switch_column(file)
    ...
    @event.class.log_result(@event, "ApplicationReportWorker::perform -> find switch_column -> found switch_column is '#{@num}'")
...
# imperative: there is only one line in servers.csv that matches server
    ...
    @event.class.log_result(@event, "ApplicationReportWorker::perform -> DEBUG: find line_containing_server -> found line is '#{@server_line}'")
# fix the line so that it only contains one line - this is a hack!
    @server_line = @server_line.match(/(?:[^|]*\|){#{@num_col - 1}}[^|]*/).to_s

    @event.class.log_result(@event, "ApplicationReportWorker::perform -> find line_containing_server -> found line is '#{@server_line}'")
...
# use hash with name = switch name and value = version.tar 
    ...
    @event.class.log_result(@event, "ApplicationReportWorker::perform -> process_file -> executing: replacing '#{@oldline}' with '#{@newline}' in file '#{file.to_s}'")
  end
  ...
  def do_perform(event_id)
    @report = ApplicationReport.find(event_id)
    @config = SystemConfiguration.in_effect

    raise UserTerminatedException unless check_terminated("ApplicationReportWorker", @event)

#    @event.class.log_result(@event, "ApplicationReportWorker::perform -> start")

    if @config.debug_mode?
#      @event.class.log_result(@event, "ApplicationReportWorker::perform -> debug mode")
    else
      @svn_path = @report.svn_path
      @svn_file = @report.svn_file
      @marker = "marker"

      Dir.mktmpdir do |dir|
        @task = "svn co #{@svn_path} #{dir}"

        raise UnixFailedExitException unless perform_unix("ApplicationReportWorker", @event, @task)
    ...
    @event.class.log_result(@event, "ApplicationReportWorker::perform -> end")
    @event.class.log_result(@event, "ApplicationReportWorker::perform -> SUCCESS: [#{@result}]")
    @event.proceed!
  end
